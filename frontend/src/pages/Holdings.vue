<template>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="row in advice" :key="row.name">
            ...
            <td>{{ row.pnl_ratio_text || pct(row.pnl_ratio) }}</td><td>{{ row.category }}</td><td><span class="tag" :class="riskClass(row.risk_level)">{{ row.scenario }}</span></td>
            <td>{{ row.trim_trigger }}</td><td>{{ row.stop_trigger }}</td><td>{{ row.add_reference }}</td><td>{{ row.action_advice }}<br><span class="muted">{{ row.reason }}</span></td>
          </tr>
        </tbody>
      </table>
    </div>
  ...
  <section class="grid">
    <form class="panel" @submit.prevent="savePosition">
      <h2>新增持仓</h2>
      <div class="form-grid">
    ...
    <div class="panel">
      <h2>持仓维护</h2>
      <div v-for="p in positions" :key="p.id" class="panel">
        <h3>{{ p.name }}</h3>
        <div class="form-grid">
        ...
        <div class="row-actions"><button class="btn primary" @click="update(p)">保存</button><button class="btn danger" @click="remove(p)">删除</button></div>
      </div>
    </div>
  </section>
</template>
